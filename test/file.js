      ...
      describe('$file.yaml', function() {
        ...
        describe('#get()', function() {
          it('Convert all the file to an object', function() {
            s.createFilesFromManifest({
              'sample_dir': {'propertiesShort.yaml': 'field1: content1\nfield2:\n  subfield1: item5\n  subfield2: item6\n  subfield3: item7\n'}
            });
          ...
          it('Gets lists, dictionaries and combinations of them', function() {
            ...
            expect($file.yaml.get(testFile, 'field1')).to.be.eql('content1');
            expect($file.yaml.get(testFile, 'field2')).to.be.eql({'subfield1': 'item5', 'subfield2': 'item6', 'subfield3': 'item7'});
            expect($file.yaml.get(testFile, 'field3')).to.be.eql([{'subfield4': 'item8', 'subfield5': 'item9'}, {'subfield6': 'item10', 'subfield7': 'item11'}]);
            expect($file.yaml.get(testFile, 'list1')).to.be.eql(['item1', 'item2']);
            expect($file.yaml.get(testFile, 'list2')).to.be.eql(['item3', 'item4']);
          });
          it('Gets inner keys', function() {
            const testFile = s.normalize('sample_dir/properties.yaml');


            expect($file.yaml.get(testFile, 'field2/subfield1')).to.be.eql('item5');
            expect($file.yaml.get(testFile, '/field2/subfield1')).to.be.eql('item5');
            expect($file.yaml.get(testFile, 'field4/subfield9/subsubfield1')).to.be.eql('item13');
            expect($file.yaml.get(testFile, '/field4/subfield9/subsubfield1')).to.be.eql('item13');
            expect($file.yaml.get(testFile, 'subfield1')).to.be.eql('');
          });
          it('Infers types', function() {
            const testFile = s.normalize('sample_dir/properties.yaml');

            expect($file.yaml.get(testFile, 'integer')).to.be.eql(123);
            expect($file.yaml.get(testFile, 'float')).to.be.eql(123.4);
            expect($file.yaml.get(testFile, 'boolean')).to.be.eql(true);
            expect($file.yaml.get(testFile, 'string')).to.be.eql('example');
          });
        });
        describe('#set()', function() {
          it('Adds parameter to root', function() {
            const testFile = s.normalize('sample_dir/properties.yaml');
            const newParameter1 = 'newparameter1';
            const newParameter2 = 'newparameter2';
            const newParameter3 = 'newparameter3';
            const value1 = 'this.is.a.new.parameter';
            const value2 = {'subfield1': 'value', 'subfield2': 'anothervalue'};
            const value3 = [{'subfield1': 'value', 'subfield2': 'value2'}, {'subfield3': 'value3', 'subfield4': 'value4'}];

            $file.yaml.set(testFile, newParameter1, value1);
            expect($file.yaml.get(testFile, newParameter1)).to.be.eql(value1);

            $file.yaml.set(testFile, newParameter2, value2);
            expect($file.yaml.get(testFile, newParameter2)).to.be.eql(value2);

            $file.yaml.set(testFile, newParameter3, value3);
            expect($file.yaml.get(testFile, newParameter3)).to.be.eql(value3);

            // Check previous information is still there
            expect($file.yaml.get(testFile, 'field1')).to.be.eql('content1');
            expect($file.yaml.get(testFile, 'field2')).to.be.eql({'subfield1': 'item5', 'subfield2': 'item6', 'subfield3': 'item7'});
            expect($file.yaml.get(testFile, 'field3')).to.be.eql([{'subfield4': 'item8', 'subfield5': 'item9'}, {'subfield6': 'item10', 'subfield7': 'item11'}]);
            expect($file.yaml.get(testFile, 'field4')).to.be.eql({'subfield8': 'item12', 'subfield9': {'subsubfield1': 'item13'}});
            expect($file.yaml.get(testFile, 'list1')).to.be.eql(['item1', 'item2']);
            expect($file.yaml.get(testFile, 'list2')).to.be.eql(['item3', 'item4']);
            expect($file.yaml.get(testFile, 'integer')).to.be.eql(123);
            expect($file.yaml.get(testFile, 'float')).to.be.eql(123.4);
          ...
          it('Updates value on root', function() {
            const testFile = s.normalize('sample_dir/properties.yaml');
            const parameter1 = 'field1';
            const parameter2 = 'field2';
            const parameter3 = 'field3';
            const value1 = 'new.value';
            const value2 = 987;
            const value3 = {'key': 'value'};

            $file.yaml.set(testFile, parameter1, value1);
            expect($file.yaml.get(testFile, parameter1)).to.be.eql(value1);

            $file.yaml.set(testFile, parameter2, value2);
            expect($file.yaml.get(testFile, parameter2)).to.be.eql(value2);

            $file.yaml.set(testFile, parameter3, value3);
            expect($file.yaml.get(testFile, parameter3)).to.be.eql(value3);
