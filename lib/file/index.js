...
module.exports.contextify = function(options) {
  ...
  // copy, rename and its alias 'move' require normalizing both source and destination
  const normalizedKeys = ['copy', 'rename', 'move'];
  normalizedKeys.forEach(m => {
    obj[m] = (src, dst, opts) => module.exports[m](normalizeFile(src), normalizeFile(dst), opts);
  });

  const specialWrappableKeys = ['ini', 'yaml', 'glob', 'xml'].concat(normalizedKeys);
