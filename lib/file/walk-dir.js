...
function walkDir(file, callback, options) {
  ...
  options = _.opts(options, {followSymLinks: false, maxDepth: null});
  const prefix = options.prefix || file;
  const baseDepth = file.split(path.sep).length;

  function _walkDir(f) {
    const type = fileType(f);
    const relativePath = stripPathPrefix(f, prefix);
    const metaData = {type: type, file: relativePath};
    const depth = f.split(path.sep).length - baseDepth;
    ...
    if (type === 'directory' || (type === 'link' && options.followSymLinks && isDirectory(f, options.followSymLinks))) {
      ...
      if (options.maxDepth || options.maxDepth === 0) {
        shouldGoDeeper = depth < options.maxDepth;
      }
      if (shouldGoDeeper) {
        _.each(fs.readdirSync(f), (elem) => {
          shouldContinue = _walkDir(path.join(f, elem));
