...
function _extractValue(data, keyPath) {
  ...
  if (_.isUndefined(keyPath) || _.isNull(keyPath) || keyPath === '' || keyPath === '/') {
    ...
    if (_.every(keyPath, (k) => _.isString(k))) {
      ...
      throw new TypeError('Expected \'keyPath\' to be a string (\'/outerKey/innerKey\') or an array of strings ("[\'outerKey\', \'innerKey\']")');
    }
 ...
 * @param {string} keyPath to read (it can read nested keys: `'outerKey/innerKey'` or `'/outerKey/innerKey'`. `null` or `'/'` will match all the document). Alternative format: ['outerKey', 'innerKey'].
