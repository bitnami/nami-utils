 ...
 * @param {boolean} [options.abortOnUnmatch=false] - If enabled, an error will be thrown if any of the files in the list does not match any of the provided patterns or no file is resolved for substitution
...
function substitute(files) {
  ...
  _.each(globFile(files, globOptions), function(file) {
    ...
    if (isFile(file, {acceptLinks: true})) {
      ...
      filesToReplace = filesToReplace.concat(listDirContents(file, {onlyFiles: true, exclude: options.exclude, followSymLinks: false, listSymLinks: options.followSymLinks}));
    } else {
      // ignore it
    }
  });
