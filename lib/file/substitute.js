...
function _massFileSubstitute(files, substitutions, options) {
  options = _.opts(options, {
    ignoreBigFiles: true, bigFileThreshold: 5000000, skipBinaries: true,
    abortOnUnmatch: false
  });
  files = _.uniq(files);
...
function substitute(files) {
  const defaultOptions = {
    recursive: false, abortOnUnmatch: false, cwd: null,
  ...
  const globOptions = {cwd: options.cwd, ignore: options.exclude, absolutize: true};
  _.each(globFile(files, globOptions), function(file) {
    if (isFile(file)) {
      filesToReplace.push(file);
    } else if (isDirectory(file) && options.recursive) {
      filesToReplace = filesToReplace.concat(listDirContents(file, {onlyFiles: true, exclude: options.exclude}));
    } else {
      // ignore it
    }
  });
