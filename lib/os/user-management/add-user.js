...
function _addUserLinux(user, options) {
  ...
  if (findInPath('useradd')) { // most modern systems
    ...
    if (options.password) {
      ...
      runProgram('passwd', [user], runProgramOpts);
    }
  } else {
    if (isBusyboxCommand('adduser')) { // busybox-based systems
      ...
      args.push(user);
      runProgram('adduser', args);
      runProgramOpts.input = '';
      _.each(options.groups, (group) => {
        runProgram('addgroup', [user, group], runProgramOpts);
      });
    } else {
